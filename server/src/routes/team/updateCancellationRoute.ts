...
export const cancelOfficeHourHandler: RequestHandler = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (team.cancelledMeetings.includes({ day: cancelledDate, meeting: {start: start, end: end} })) {
      res.status(400).json({ message: "Date already cancelled" });
      return;
    }

    team.cancelledMeetings = [...team.cancelledMeetings, { day: cancelledDate, meeting: {start: start, end: end} }];
    await team.save();
    ...
    res.status(200).json({ message: "Office hour cancelled successfully", cancelledDays: team.cancelledMeetings });
  } catch (error) {
